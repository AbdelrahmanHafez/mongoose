...
function AggregationCursor(agg) {
  ...
  this._cursorRequested = false;
  this._preMiddlewareDone = false;
  this._transforms = [];
...
function _init(model, c, agg) {
  ...
  function onPreComplete(err) {
    ...
    c._preMiddlewareDone = true;
    if (!model.collection._shouldBufferCommands() || !model.collection.buffer || c._cursorRequested) {
...
function _getRawCursor(model, aggregationCursor, agg) {
  if (aggregationCursor.cursor || aggregationCursor._cursorQueued) {
    return;
  }
  if (model.collection._shouldBufferCommands() && model.collection.buffer) {
    aggregationCursor._cursorQueued = true;
    model.collection.queue.push([
      () => {
        aggregationCursor._cursorQueued = false;
        _getRawCursor(model, aggregationCursor, agg);
      }
    ]);
    return;
  }
...
AggregationCursor.prototype.close = async function close() {
  ...
  try {
    if (!this.cursor && !this.skipped) {
      if (this._error) {
        throw this._error;
      }
      const waitForCursor = once(this, 'cursor');
      _requestRawCursor(this);
      await waitForCursor;
    }
    if (this.cursor) {
...
function _requestRawCursor(ctx) {
  if (ctx.cursor || ctx._error || ctx.skipped) {
    return;
  }

  ctx._cursorRequested = true;
  if (ctx._preMiddlewareDone) {
    _getRawCursor(ctx.agg._model, ctx, ctx.agg);
  }
}
