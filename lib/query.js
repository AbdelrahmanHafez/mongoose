...
Query.prototype._castUpdate = function _castUpdate(obj, overwrite) {
  ...
  const strict = _getStrict(this);
  const omitUndefined = _getOmitUndefined(this);
  const useNestedStrict = _getUseNestedStrict(this);
  const upsert = _getUpsert(this);
...
function _getUseNestedStrict(context) {
  if ('useNestedStrict' in context.options) {
    return context.options.useNestedStrict;
  }
}

function _getUpsert(context) {
  if ('upsert' in context.options) {
    return context.options.upsert;
  }
}

function _getOmitUndefined(context) {
  if ('omitUndefined' in context._mongooseOptions) {
    return context._mongooseOptions.omitUndefined;
  }

  return false;
}

function _getStrict(context) {
  if ('strict' in context._mongooseOptions) {
    return context._mongooseOptions.strict;
  }

  if (context.schema && context.schema.options) {
    return context.schema.options.strict;
  }
