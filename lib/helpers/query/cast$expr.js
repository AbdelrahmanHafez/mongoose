...
function castComparison(val, schema, strictQuery) {
  ...
  if (isLiteral(val[1])) {
    ...
    if (isPath(lhs)) {
      ...
      for (const dateExpression of dateExpressions) {
        const potentialPath = lhs[dateExpression];
        if (isPath(potentialPath)) {
          path = potentialPath.slice(1) + '.' + dateExpression;
          caster = castNumber;
          break;
        }
      }
